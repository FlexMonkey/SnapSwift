...
class SnapSwiftParameterWidget: UIView
{
    ...
    {
        titleLabel.frame = CGRect(x: 0, y: 0, width: frame.width / 2 + 10, height: frame.height).insetBy(dx: 4, dy: 0)
        valueLabel.frame = CGRect(x: frame.width / 2, y: 0, width: frame.width / 2, height: frame.height).insetBy(dx: 4, dy: 0)
        
        progressView.frame = CGRect(x: 0, y: frame.height - 5, width: frame.width, height: 0).insetBy(dx: 5, dy: 0)
        
        backgroundLayer.frame = CGRect(x: 0, y: 0, width: frame.width, height: frame.height).insetBy(dx: 0, dy: 0.5)
